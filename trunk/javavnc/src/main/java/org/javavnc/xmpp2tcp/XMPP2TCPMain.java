...
public class XMPP2TCPMain {
    ...
    private static Hashtable<String, String> threadIds = new Hashtable<String, String>();
    ...
    public static void main(String[] args) throws XMPPException {
        ...
        logger.info("Host:" + host + "; port:" + port + "; sender:" + sender);

        /*
         * try to get the connection, if failed, wait 1s and try again until we
         * success
         */
        long waitTime = 1000;
        do {
            con = new XMPPConnectionManager().getGoogleConnection();
            if (con == null) {
                try {
        ...
        // Verify this code to make sure we can relaunch this server when error
        // happens
        ...
        if (Integer.parseInt(port) == 5900) {
            try {
                // new VNCManager().installVNC();
                new VNCManager().startVNCServer();
            } catch (Exception e) {
                e.printStackTrace();
            }
        }
